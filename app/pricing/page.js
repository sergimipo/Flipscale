  ...
  const handleCheckout = async () => {
    ...
    try {
      const res = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({ origin: window.location.origin, userId: user.id }),
      });
  ...
  return (
    <div className={`min-h-screen ${c.page}`}>
      ...
      <main className="mx-auto max-w-5xl px-6 py-16">
        {isPro ? (
          /* ---------- YA TIENES PRO ---------- */
          <div className={`mx-auto max-w-lg rounded-2xl border p-10 text-center shadow-2xl ${c.card}`}>
            ...
            <h1 className="font-display text-3xl font-extrabold">Ya tienes Flipscale Pro</h1>
            <p className={`mt-3 text-sm leading-relaxed ${c.sub}`}>
              Tu licencia de por vida está activa en la cuenta{' '}
              <span className="font-semibold">{user?.email}</span>. Disfruta de todas las
          ...
          <>
            <div className="text-center">
              ...
              <p className={`mx-auto mt-5 max-w-2xl text-lg ${c.sub}`}>
                Olvídate de cuotas mensuales. Paga una vez y accede a todas las herramientas
                de Flipscale de por vida.
              </p>
            </div>

            <div className="mt-14 grid gap-8 lg:grid-cols-5">
              {/* TARJETA DE PLAN */}
              <div className={`relative overflow-hidden rounded-2xl border p-8 shadow-2xl lg:col-span-2 ${c.card}`}>
                ...
                <div className="relative">
                  ...
                  <h2 className="font-display text-2xl font-bold">Licencia de por vida</h2>
                  <p className={`mt-1 text-sm ${c.sub}`}>Todo lo que un reseller necesita.</p>
                  ...
                  {!user && (
                    <div className="mb-4">
                      <label className={`mb-1.5 block text-xs font-semibold ${c.sub}`}>Tu email</label>
                      <input
                        type="email"
                        value={email}
                      ...
                      />
                      <p className={`mt-1.5 text-xs ${c.faint}`}>
                  ...
                  <button onClick={handleCheckout} disabled={processing} className="btn-primary w-full disabled:opacity-50">
                    {processing ? 'Redirigiendo a Stripe…' : !user ? 'Crear cuenta y comprar' : 'Comprar por 5 €'}
                  </button>
              ...
              <div className="lg:col-span-3">
                <p className={`mb-5 text-sm font-semibold uppercase tracking-wider ${c.faint}`}>Todo incluido</p>
                <div className="grid gap-4 sm:grid-cols-2">
                  {features.map((f) => (
                    <div key={f.title} className={`rounded-xl border p-4 transition hover:border-brand-500/40 ${c.card}`}>
            ...
            {/* FAQ */}
            <div className="mt-20">
              <h2 className="text-center font-display text-2xl font-bold md:text-3xl">Preguntas frecuentes</h2>
              <div className="mx-auto mt-8 max-w-3xl space-y-3">
                {[
                  ...
                  <details key={item.q} className={`group rounded-xl border p-4 transition open:border-brand-500/40 ${c.card}`}>
      ...
      <footer className={`border-t px-6 py-8 ${dark ? 'border-white/5' : 'border-slate-200'}`}>
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-3 text-xs md:flex-row">
          <div className="flex items-center gap-2">
            <Logo className="h-6 w-6" />
            <span className={c.faint}>© 2026 Flipscale</span>
          </div>
          <div className={`flex gap-4 ${c.faint}`}>
            <Link href="/" className="hover:text-brand-500">Inicio</Link>
            <Link href="/login" className="hover:text-brand-500">Entrar</Link>
          </div>
        </div>
      </footer>
    </div>
