          ...
          {magicSent ? (
            <div className="text-center">
              ...
              <p className="mt-2 text-sm leading-relaxed text-slate-400">
                Te hemos enviado un enlace mágico a <span className="font-semibold text-brand-400">{email}</span>.
                Ábrelo para entrar.
              </p>
              <button
            ...
            <>
              ...
              {/* TOGGLE: Email / Contraseña */}
              <div className="mb-4 flex rounded-lg bg-ink-800/60 p-1">
                <button
                  onClick={() => setShowPassword(false)}
                  className={`flex-1 rounded-md px-3 py-1.5 text-xs font-semibold transition ${
                    !showPassword ? 'bg-white/10 text-white' : 'text-slate-400'
                  }`}
                >
                  Enlace mágico
                </button>
                ...
                <form onSubmit={handleMagicLink} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-white/10 bg-ink-800/60 px-4 py-3 text-sm outline-none transition placeholder:text-slate-500 focus:border-brand-500"
                    />
                    <p className="mt-1.5 text-xs text-slate-500">Te enviaremos un enlace para entrar sin contraseña.</p>
                  </div>
                  {error && (
                    <p className="rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-xs text-red-400">{error}</p>
                  )}
